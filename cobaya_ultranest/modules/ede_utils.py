...
def save_results(result, sampler, param_names, param_labels, filename='ede_results.pkl'):
    """
...
def create_triangle_plot(samples, weights, names, labels, output_file):
    ...
    try:
        gd_samples = MCSamples(samples=samples, weights=weights, names=names, labels=labels, label='EDE Model')
