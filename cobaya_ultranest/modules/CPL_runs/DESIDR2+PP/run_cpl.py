...
# Import our modules
import cpl_config as cfg
import cpl_utils as utils

def main():
    print("========================================")
    print("   CPL COSMOLOGY ANALYSIS (Modular)     ")
    print("========================================")

    # 1. Initialize logic helpers
    #    The wrapper handles the Cobaya model internally
    cpl_wrapper = utils.cplModelWrapper(cfg.COBAYA_INFO, cfg.PARAM_NAMES)
    ...
    try:
        logl = cpl_wrapper.log_likelihood(fiducial_point)
    ...
    sampler = ultranest.ReactiveNestedSampler(
        ...
        loglike=cpl_wrapper.log_likelihood,
    ...
    # 5. Save and Plot
    print("\n--- Processing Results ---")
    pkl_path = os.path.join(cfg.OUTPUT_DIR, "cpl_results.pkl")
    plot_path = os.path.join(cfg.OUTPUT_DIR, "cpl_triangle.pdf")
