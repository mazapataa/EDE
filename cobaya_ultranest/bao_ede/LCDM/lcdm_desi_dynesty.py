...
# ---------------------------
# Cobaya configuration (info) - LCDM parameterization with H0 sampling
# ---------------------------
info = {
    "theory": {
        "classy": {
...
# LCDM sampled parameters - NOW WITH H0 INSTEAD OF theta_s_100
param_names = ['logA', 'n_s', 'H0', 'omega_b', 'omega_cdm', 'tau_reio']
param_labels = [r'$\log(10^{10}A_s)$', r'$n_s$', r'$H_0$', r'$\Omega_b h^2$', 
                r'$\Omega_c h^2$', r'$\tau_{reio}$']
param_labels_latex = [r'\log(10^{10} A_\mathrm{s})', 'n_\mathrm{s}', r'H_0',
                      r'\Omega_\mathrm{b} h^2', r'\Omega_\mathrm{c} h^2', r'\tau_\mathrm{reio}']
...
# ---------------------------
# Prior transform for dynesty - H0 VERSION
# ---------------------------
def prior_transform(u):
    """Transform unit cube [0,1]^6 to parameter space with physically motivated priors
    
    Now sampling H0 directly instead of theta_s_100.
...
def log_likelihood(theta):
    ...
    try:
        ...
        if fail_count % 50 == 0 and rank == 0:
            ...
            print(f"Parameters: H0={params_dict['H0']:.2f}, omega_b={params_dict['omega_b']:.4f}, omega_cdm={params_dict['omega_cdm']:.4f}")
        return -np.inf
...
# ---------------------------
def run_nested_sampling(ndim=6, nlive=500, dlogz=0.005, output_dir='./lcdm_dynesty_H0_output'):
    ...
    print_rank0("\n" + "="*60)
    print_rank0("Starting Dynesty Nested Sampling - LCDM with H0 sampling")
    print_rank0("="*60)
    ...
    # Show the actual tight priors being used
    tight_priors = {
        'logA': (2.7, 3.3),
        'n_s': (0.92, 1.01),
        'H0': (62.0, 77.0),
    ...
    for pname in param_names:
        bounds = tight_priors[pname]
        fid = fiducial[pname]
        print_rank0(f"  {pname:12s}: [{bounds[0]:.4f}, {bounds[1]:.4f}] (fiducial: {fid:.4f})")
    
    print_rank0("\n  Note: Sampling H0 directly (not theta_s_100)")
    print_rank0("        Prior range encompasses CMB (~67) and local (~73) measurements")
...
def save_chains_and_info(results, output_dir='./lcdm_dynesty_H0_output'):
    ...
    # Show the actual tight priors used
    tight_priors_used = {
        'logA': (2.7, 3.3),
        'n_s': (0.92, 1.01),
        'H0': (62.0, 77.0),
    ...
    info_dict = {
        ...
        'sampling_priors': {k: list(v) for k, v in tight_priors_used.items()},
        'cobaya_bounds': {k: list(v) for k, v in prior_bounds.items()},
        'fiducial_values': fiducial,
        'mpi_processes': size,
        'success_count': success_count,
        'fail_count': fail_count,
        'sampling_mode': 'H0_direct',
        'note': 'Sampling H0 directly instead of theta_s_100'
    }
...
def analyze_results(results, output_dir='./lcdm_dynesty_H0_output'):
    ...
    print_rank0("\n" + "="*60)
    print_rank0("RESULTS SUMMARY - LCDM WITH H0 SAMPLING")
    print_rank0("="*60)
    ...
    summary_lines = ["Parameter Estimates (H0 sampling mode):", "-" * 80]
...
if __name__ == "__main__":
    ...
    output_dir = './lcdm_dynesty_H0_output'
    
    # Run nested sampling
    results = run_nested_sampling(ndim=6, nlive=500, dlogz=0.005, output_dir=output_dir)
