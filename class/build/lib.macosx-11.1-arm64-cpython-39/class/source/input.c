...
int input_read_parameters_species(struct file_content * pfc,
  ...
  if (pba->Omega0_fld != 0.) {
    ...
    if (flag1 == _TRUE_) {
      ...
      else if ((strstr(string1,"nCPL") != NULL) || (strstr(string1,"ncpl") != NULL)) { /*Modification*/
        pba->fluid_equation_of_state = nCPL;
      }
    ...
    if (pba->fluid_equation_of_state == nCPL) {
      /**Modification: Equation of state of the fluid in '7CPL' case */
      /* Read */
      class_read_double("w0_fld",pba->w0_fld);
      class_read_double("wa_new",pba->wa_new);
      class_read_double("cs2_fld",pba->cs2_fld);
    }
...
int input_prepare_pk_eq(struct precision * ppr,
  ...
  double true_w0_fld;
  double true_wa_fld;
  /*double true_wb_fld;Modification?*/
  double * z;
...
int input_read_parameters_primordial(struct file_content * pfc,
  ...
  class_read_double("Amp_prim",ppm->Amp_prim);
  class_read_double("Delta_prim",ppm->Delta_prim);
...
int input_default_params(struct background *pba,
  ...
  pba->fluid_equation_of_state = CLP;
  pba->fluid_equation_of_state = nCPL; /**Modification '7CPL'*/
  pba->w0_fld = -1.;
  pba->cs2_fld = 1.;
  /** 9.a.2.1) 'CLP' case */
  pba->wa_fld = 0.;
